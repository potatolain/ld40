...
void banked_draw_hud_to_b() {
    // Presumes ppu is already off.
    // Fill all palettes out
    vram_adr(NAMETABLE_B + 0x03f0);
    for (i = 0; i < 16; i++) {
        vram_put(0x55);
    }

    vram_adr(NTADR_B(0, 24));
    vram_put(HUD_TL);
    vram_fill(HUD_H, 30);
    vram_put(HUD_TR);
    for (i = 0; i < 3; i++) {
